...
class CommitmentSpecSpec {
  def allTests = {

    {
      ...
      val spec1 = CommitmentSpec.reduce(spec, Vector(Htlc(false, add1, null, null, "")), Vector.empty)
      println(spec1 == spec.copy(htlcs = Set(Htlc(false, add1, null, null, "")), toLocalMsat = 3000 * 1000))

      val add2 = UpdateAddHtlc("00" * 32, 2, 1000 * 1000, 400, H, "")
      val spec2 = CommitmentSpec.reduce(spec1, Vector(Htlc(false, add2, null, null, "")), Vector.empty)
      println(spec2 == spec1.copy(htlcs = Set(Htlc(false, add1, null, null, ""), Htlc(false, add2, null, null, "")), toLocalMsat = 2000 * 1000))
...

    {
      ...
      val add1 = UpdateAddHtlc("00" * 32, 1, 2000 * 1000, 400, H, "")
      val spec1 = CommitmentSpec.reduce(spec, Vector.empty, Vector(Htlc(true, add1, null, null, "")))
      println(spec1 == spec.copy(htlcs = Set(Htlc(true, add1, null, null, "")), toRemoteMsat = 3000 * 1000))

      val add2 = UpdateAddHtlc("00" * 32, 2, 1000 * 1000, 400, H, "")
      val spec2 = CommitmentSpec.reduce(spec1, Vector.empty, Vector(Htlc(true, add2, null, null, "")))
      println(spec2 == spec1.copy(htlcs = Set(Htlc(true, add1, null, null, ""), Htlc(true, add2, null, null, "")), toRemoteMsat = 2000 * 1000))
