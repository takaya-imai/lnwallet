...
/**
 * Poly1305New message authentication code, designed by D. J. Bernstein.
 * <p>
 * Poly1305New computes a 128-bit (16 bytes) authenticator, using a 128 bit nonce and a 256 bit key
        ...
        implements Mac
{
    ...
    /**
     * Constructs a Poly1305New MAC, where the key passed to init() will be used directly.
     */
    public Poly1305New()
    {
        this.cipher = null;
    ...
    /**
     * Constructs a Poly1305New MAC, using a 128 bit block cipher.
     */
    public Poly1305New(final BlockCipher cipher)
    {
        if (cipher.getBlockSize() != BLOCK_SIZE)
        {
            throw new IllegalArgumentException("Poly1305New requires a 128 bit block cipher.");
        }
        this.cipher = cipher;
    }

    /**
     * Initialises the Poly1305New MAC.
     *
     ...
     *        {@link Poly1305KeyGenerator Poly1305New key format}, otherwise just the
     *        {@link KeyParameter}.
     */
            ...
            throws IllegalArgumentException
    {
        ...
        if (cipher != null)
        {
            ...
            {
                throw new IllegalArgumentException("Poly1305New requires an IV when used with a block cipher.");
            }
        ...
        {
            throw new IllegalArgumentException("Poly1305New requires a key.");
        }
    ...
    private void setKey(final byte[] key, final byte[] nonce)
    {
        if (key.length != 32)
        {
            throw new IllegalArgumentException("Poly1305New key must be 256 bits.");
        }
        if (cipher != null && (nonce == null || nonce.length != BLOCK_SIZE))
        {
            throw new IllegalArgumentException("Poly1305New requires a 128 bit IV.");
        }
    ...
    {
        return cipher == null ? "Poly1305New" : "Poly1305New-" + cipher.getAlgorithmName();
    }
